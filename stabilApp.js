...
export default function App() {
  ...
  const [sosyalMedya, setSosyalMedya] = useState({ facebook: '', twitter: '', instagram: '', youtube: '', whatsapp: '', adres: '' });
  ...
  const [gizliSifre, setGizliSifre] = useState('nemesis123');
  ...
  return (
    <ScrollView style={styles.anaScrollKonteyner} contentContainerStyle={styles.anaScrollIcerik} showsVerticalScrollIndicator={true}>
        ...
        <View style={{ width: '100%' }}>
          ...
          {yonetimKurulu.length > 0 ? (
            <View style={styles.kurumsalYonetimKadroBloku}>
              <TouchableOpacity style={styles.akordeonBaslikAlani} activeOpacity={0.7} onPress={() => setUyeYonetimAcikMi(!uyeYonetimAcikMi)}>
                <Text style={styles.yonetimKadroBaslik}>👥 ORGANİZASYON ŞEMASI & YÖNETİM</Text>
          ...
          <View style={styles.kurumsalFooterSeridi}>
            <View style={styles.footerIkonSatiri}>
              {['facebook', 'twitter', 'instagram', 'youtube', 'whatsapp', 'adres'].map((p) => sosyalMedya[p] ? (
          ...
          {!adminGirisYaptiMi ? (
            <View style={styles.loginKonteyner}>
              ...
              <View style={styles.satir}>
                <TouchableOpacity style={[styles.kaydetButon, {flex: 2, backgroundColor: '#00205B'}]} onPress={() => { if(girilenSifre===gizliSifre){setAdminGirisYaptiMi(true); setGirilenSifre('');}else{alert("Hatalı!");} }}>
            ...
            <View style={{ width: '100%' }}>
              ...
              <View style={styles.adminBlok}>
                <TouchableOpacity style={styles.akordeonBaslikAlani} onPress={() => setFormAcikMi(!formAcikMi)}>
                  <Text style={styles.blokBaslikDokunmatik}>{duzenlenenId ? '✏️ Kurum Sözleşme Uzatma / Güncelleme' : '➕ Yeni Evrak ve Sözleşme Yükle'}</Text>
      ...
      <Modal visible={sosyalModalAcikMi} transparent={true} animationType="fade">
        <View style={styles.modalArkaPlan}>
          <View style={[styles.dropdownMenuKonteyner, { width: 360 }]}>
            ...
            <Text style={styles.inputEtiket}>YouTube Kanal Linki</Text>
            <TextInput style={styles.inputField} value={inputYt} onChangeText={setInputYt} placeholder="youtube.com/@kanali" placeholderTextColor="#999" />
      ...
      {/* 👥 MODAL: ESNEK VE TAM DİNAMİK YÖNETİM KURULU YAPILANDIRMASI */}
      <Modal visible={yonetimModalAcikMi} transparent={true} animationType="fade">
        <View style={styles.modalArkaPlan}>
          <View style={[styles.dropdownMenuKonteynerGenisletilmis, { maxHeight: '85%' }]}>
            ...
            {/* ⚡ YENİ DÜZENLEME: GÖREVLİ EKLE BUTONU BÖLÜMÜN EN ALTINA (KAYDET BUTONLARININ HEMEN ÜSTÜNE) TAŞINDI */}
...
const styles = StyleSheet.create({
  ...
  // 🎨 RESTORED: ORİJİNAL DİKEY HİZALI EN ÇOK BEĞENİLEN YEŞİL AVANTAJ BLOK STİLLERİ
